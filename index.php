...
class BackendCoder
{
    ...
    public function __construct($wmc)
    {
        if (!$wmc->looking_for_a_challenging_job()) {
            throw new Exception(
                'Try google, facebook or microsoft.',
                'Looking_For_Something_Else_Big_And_Safe_Exception'
            );
        }
        ...
        if (!$wmc->knows_php_to_fingertips()) {
            if ($wmc->really_eager_to_learn() && $wmc->has_junior_potential()) {
                throw new Exception(
                    'Send an email to the boss: nils-fredrik@maksimer.no with a few lines and exception code',
                    'Not_Qualified_But_Might_Be_A_Quick_Learning_Talent'
                );
            }
        ...
        if (!$wmc->can_wear_a_lumberjack_shirt_every_friday()) {
            throw new Exception(
                'Sorry. Company policy. Try some wannabe hipster agency where you can wear square glasses and bow tie.',
                'Social_Lumberjack'
            );
        ...
        if (!$wmc->will_code_for_food()) {
            throw new Exception(
                'Our lunch is actually (almost…) free and everyone loves a good burger!.',
                'Code_For_Food'
            );
        }
        ...
        if (!$wmc->will_work_hard_when_required() || !$wmc->sees_producing_great_code_as_a_reward()) {
            throw new Exception(
                'Sorry. If champagne is your reward, try a creative design agency.',
                'Social_Hardworking'
            );
        }
    ...
    public function contact()
    {
        $wmc->email_cv_and_short_summary(
            ...
            'Backend developer',
            'Hi, attached you find my CV. Here you get 3 reasons you should hire me for your awesome PHP, ' .
            'WordPress / WooCommerce position: '
        );
    }
}
